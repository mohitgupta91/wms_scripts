...
query = """
        ...
        FROM wms.inventory inv
            ...
            LEFT JOIN wms.qc qc ON inv.barcode = qc.barcode
...
for row in data:
    # query = """ SELECT qc_date_time from qc WHERE barcode ='""" + row[0] + """';"""
    ...
    if row[9] is None:
        query = """
            SELECT cri.code,CAST(crih.created AS char) AS 'QC Date'
                ...
                JOIN rms.cri_history crih ON crih.cri_id = cri.id
                    AND crih.next_status_code <> crih.previous_status_code
                    AND crih.next_status_code = 'PDC'
        ...
        if row_count_rms == 0:
            ...
            query = """ INSERT INTO qc (barcode, qc_date_time) VALUES """ + data_rms
...
query = """
        ...
        ROUND(COUNT(DISTINCT CASE WHEN DATEDIFF(rtv.updated, qc.qc_date_time)<=1 THEN inv.id END)/COUNT(DISTINCT inv.id)*100,1) AS '%1 day',
        ROUND(COUNT(DISTINCT CASE WHEN DATEDIFF(rtv.updated, qc.qc_date_time)<=2 THEN inv.id END)/COUNT(DISTINCT inv.id)*100,1) AS '%2 day',
        ROUND(COUNT(DISTINCT CASE WHEN DATEDIFF(rtv.updated, qc.qc_date_time)<=3 THEN inv.id END)/COUNT(DISTINCT inv.id)*100,1) AS '%3 day',
        ROUND(COUNT(DISTINCT CASE WHEN DATEDIFF(rtv.updated, qc.qc_date_time)<=5 THEN inv.id END)/COUNT(DISTINCT inv.id)*100,1) AS '%5 day',
        ROUND(COUNT(DISTINCT CASE WHEN DATEDIFF(rtv.updated, qc.qc_date_time) >5 THEN inv.id END)/COUNT(DISTINCT inv.id)*100,1) AS '% >5 day',
        COUNT(DISTINCT inv.id) AS 'Total Dispatch'

        FROM wms.inventory inv
        JOIN wms.qc qc ON qc.barcode = inv.barcode
        JOIN wms.rtv_sheet_detail_inventory rts ON inv.id = rts.productDetails_id
        JOIN wms.rtv_sheet_detail rtv ON rts.rtv_sheet_detail_id = rtv.id
        LEFT JOIN wms.users u ON u.id  = rtv.updatedBy_id
        LEFT JOIN wms.warehouse wh ON wh.id = rtv.warehouse_id
...
html_data += '<br><br>Please find QC To RTS Report.</strong><br>'
