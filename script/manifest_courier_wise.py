	...
	for wh in warehouseList:
		query = """
                	...
                	WHERE manifest.courier_code = '""" + courier[0] + """'
                    	AND manifest.created BETWEEN NOW() - INTERVAL 1 DAY AND NOW()  and warehouse.name='"""+wh[0]+"""';
                	"""
		cursor.execute(query)
        ...
        query = """Update manifest m set m.is_email_sent = 1 WHERE m.courier_code = '""" + courier[0] + """'; """
       # cursor.execute(query)
        db.commit()
db.close()
